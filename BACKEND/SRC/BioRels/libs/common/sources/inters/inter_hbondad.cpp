...
protspace::Inter_HBond_AD::Inter_HBond_AD(MacroMole &pMole, const bool &pIsSameMole):
...
bool protspace::Inter_HBond_AD::checkInteraction()
{
    try{
        if (!getAtomRef() .prop().hasProperty(CHEMPROP::HBOND_ACC))return false;
        if (!getAtomComp().prop().hasProperty(CHEMPROP::HBOND_DON))return false;
...
bool protspace::Inter_HBond_AD::checkAngle()
try{

    double currAngle, diff = 100, lowerdiff = 1000;
    protspace::MMAtom& atomD=getAtomComp();
    protspace::MMAtom& atomA=getAtomRef();
