...
addLog("Working directory: ".$W_DIR);
	...
	$res=runQuery($query);
	if ($res===false)																	failProcess($JOB_ID."006",'Unable to get the list of genes');
	$list_g=array();
	...
	$fp=fopen('uniprot_mapping.csv','w');if (!$fp)										failProcess($JOB_ID."007",'Unable to open uniprot_mapping.csv');
	...
	foreach ($CHUNKS as $NC=>$CHUNK)
	{
		...
		$res=runQuery($query);
		echo count($res)."\t";
		if ($res===false)																	failProcess($JOB_ID."008",'Unable to fetch uniprot mapping');
		$NV=0;
		foreach ($res as $line)
		{
	...
	if (!is_dir("DATA") && !mkdir("DATA"))												failProcess($JOB_ID."009",'Unable to create jobs directory');
	...
	for($I=0;$I<$N_JOB;++$I)
	{
		$COMMANDS[$I+1]='biorels_php '.$RUNSCRIPT.' '.$I.' '.$N_JOB.' &> LOG_'.$I;
	}
