...
addLog("Create directory");
	...
	/// Get parent info
	$PMJ_INFO=$GLB_TREE[getJobIDByName('pmj_dbsnp')];

	/// Set working directory
	$W_DIR=$TG_DIR.'/'.$GLB_VAR['PROCESS_DIR'];  if (!is_dir($W_DIR)) 					failProcess($JOB_ID."001",'NO '.$W_DIR.' found ',$PROCESS_CONTROL_JOB);
	$W_DIR.='/'.$JOB_INFO['DIR'].'/DBSNP/';   	 if (!is_dir($W_DIR) && !mkdir($W_DIR)) failProcess($JOB_ID."002",'Unable to find and create '.$W_DIR,$PROCESS_CONTROL_JOB);
...
function rmj_dbsnp_term()
{
	...
	$W_DIR.='/'.$PMJ_INFO['DIR'].'/DBSNP/';   		if (!is_dir($W_DIR))			 		failProcess($JOB_ID."012",'Unable to find  '.$W_DIR,$PROCESS_CONTROL_JOB);
	$W_DIR.='/'.$PMJ_INFO['TIME']['DEV_DIR'].'/';   if (!is_dir($W_DIR))				 	failProcess($JOB_ID."013",'Unable to find  '.$W_DIR,$PROCESS_CONTROL_JOB);
	...
	for ($I=0;$I<$LC;++$I)
	{
		if (!checkFileExist($W_DIR.'/SCRIPTS/status_'.$I))$SUCCESS=false;
		if (file_get_contents($W_DIR.'/SCRIPTS/status_'.$I)!=0)$SUCCESS=false;
	}
