...
addLog("Create directory");
	...
	$PMJ_INFO=$GLB_TREE[getJobIDByName('pmj_alfa')];
	$W_DIR=$TG_DIR.'/'.$GLB_VAR['PROCESS_DIR'];  if (!is_dir($W_DIR)) 					failProcess($JOB_ID."001",'NO '.$W_DIR.' found ');
	$W_DIR.='/'.$JOB_INFO['DIR'].'/DBSNP/';   	 if (!is_dir($W_DIR) && !mkdir($W_DIR)) failProcess($JOB_ID."002",'Unable to find and create '.$W_DIR);
	$W_DIR.='/'.$PMJ_INFO['TIME']['DEV_DIR'].'/';if (!is_dir($W_DIR) && !mkdir($W_DIR)) failProcess($JOB_ID."003",'Unable to find and create '.$W_DIR);
	if (!chdir($W_DIR) )															 	failProcess($JOB_ID."004",'Unable to find and create '.$W_DIR);
	...
	$ALL_FILE=$W_DIR.'/all_alfa.sh';
	if (!is_file($ALL_FILE))															failProcess($JOB_ID."009",'Unable to find master job file at '.$ALL_FILE,$PROCESS_CONTROL_JOB);
	
	
	$LC=getLineCount($ALL_FILE);
	
	
	exec('qsub -tc '.$LC.' -o '.$TG_DIR.'/BACKEND/LOG/SGE_LOG/TG_'.$JOB_ID.'_'.date("Y_m_d_H_i_s").'.o -e '.
						   $TG_DIR.'/BACKEND/LOG/SGE_LOG/TG_'.$JOB_ID.'_'.date("Y_m_d_H_i_s").'.e -v TG_DIR '.
						   '-N '.$GLB_VAR['JOB_PREFIX'].'_'.$JOB_ID.' -t 1-'.$LC.':1 '.$JOBARRAY.' '.$ALL_FILE,$res,$return_code);
			if (!$return_code!=0)														failProcess($JOB_ID."010",'Unable to find master job file at '.$ALL_FILE,$PROCESS_CONTROL_JOB);
...
addLog("Create directory");
	...
	$W_DIR=$TG_DIR.'/'.$GLB_VAR['PROCESS_DIR'];if (!is_dir($W_DIR)) 						failProcess($JOB_ID."001",'NO '.$W_DIR.' found ');
	$W_DIR.='/'.$JOB_INFO['DIR'].'/DBSNP/';   		if (!is_dir($W_DIR))			 		failProcess($JOB_ID."002",'Unable to find and create '.$W_DIR);
	$W_DIR.='/'.$PMJ_INFO['TIME']['DEV_DIR'].'/';   if (!is_dir($W_DIR))				 	failProcess($JOB_ID."003",'Unable to find and create '.$W_DIR);
	...
	$ALL_FILE=$W_DIR.'/SCRIPTS/all_alfa.sh';
...
function rmj_alfa_term()
{
	...
	$W_DIR=$TG_DIR.'/'.$GLB_VAR['PROCESS_DIR'];if (!is_dir($W_DIR)) 						failProcess($JOB_ID."011",'NO '.$W_DIR.' found ',$PROCESS_CONTROL_JOB);
	$W_DIR.='/'.$PMJ_INFO['DIR'].'/DBSNP/';   		if (!is_dir($W_DIR))			 		failProcess($JOB_ID."012",'Unable to find and create '.$W_DIR,$PROCESS_CONTROL_JOB);
	$W_DIR.='/'.$PMJ_INFO['TIME']['DEV_DIR'].'/';   if (!is_dir($W_DIR))				 	failProcess($JOB_ID."013",'Unable to find and create '.$W_DIR,$PROCESS_CONTROL_JOB);
	


	/// Checking master script:
	$ALL_FILE=$W_DIR.'/all_alfa.sh';
	if (!is_file($ALL_FILE))															failProcess($JOB_ID."014",'Unable to find master job file at '.$ALL_FILE,$PROCESS_CONTROL_JOB);
	...
	for ($I=0;$I<$LC;++$I)
	{
		if (!checkFileExist($W_DIR.'/status_ALFA_'.$I))$SUCCESS=false;
		if (file_get_contents($W_DIR.'/status_ALFA_'.$I)!=0)$SUCCESS=false;
