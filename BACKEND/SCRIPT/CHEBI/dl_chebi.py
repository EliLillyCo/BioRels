...
#Get parent job info
CK_INFO=GLB_TREE[get_job_id_by_name('ck_chebi_rel')]
	...
	while line:
		...
		#	print("TAB7: +"+st[5])
			if (st[5]=='[TXT]' or st[5]=='[   ]'):
				...
				if (not dl_file(GLB_VAR['LINK']['FTP_CHEBI']+"/Flat_file_tab_delimited/"+st[7],3,st[7])):
					fail_process(JOB_ID + '006', 'Unable to download '+st[7])
		line=f.readline()
f.close()

add_log("\tDownloading ChEBI Complete")
if (not dl_file(GLB_VAR['LINK']['FTP_CHEBI']+"/SDF/ChEBI_complete.sdf.gz",3)):
	fail_process(JOB_ID + '007', 'Unable to download SDF/ChEBI_complete.sdf.gz')

if (not dl_file(GLB_VAR['LINK']['FTP_CHEBI']+"/SDF/ChEBI_complete_3star.sdf.gz",3)):
	fail_process(JOB_ID + '008', 'Unable to download SDF/ChEBI_complete_3star.sdf.gz')
