...
#Get parent job info
CK_INFO=GLB_TREE[get_job_id_by_name('dl_chebi')]
...
SOURCE_ID=get_source("ChEBI",True)	


list_files=['compounds.tsv.gz','compounds_3star.tsv.gz','ChEBI_complete_3star.sdf.gz','names.tsv.gz','names_3star.tsv.gz','reference.tsv.gz','reference_3star.tsv.gz','structures_3star.csv.gz','ChEBI_complete.sdf.gz'];
for file in list_files:
	if (check_file_exist(file)):
		add_log("Uncompressing "+file)
		if (not ungzip(file)):
			fail_process(JOB_ID + '005', 'Unable to uncompress '+file)

if (check_file_exist('chebi_uniprot.zip')):
	add_log("Uncompressing chebi_uniprot")
	if (not unzip('chebi_uniprot.zip')):
		fail_process(JOB_ID + '005', 'Unable to uncompress chebi_uniprot.zip')
	os.remove('chebi_uniprot.zip')
...
convert('ChEBI_complete_3star.sdf')
