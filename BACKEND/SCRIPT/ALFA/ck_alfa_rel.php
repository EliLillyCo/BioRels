...
addLog("Working directory: ".$W_DIR);
	...
	/// Remove README.txt file if it already exists
	if (is_file('README.txt'))unlink($W_DIR.'/README.txt');

	/// Download file
	if (!dl_file($WLINK.'/README.txt',3))										failProcess($JOB_ID."005",'Unable to download README.txt');
	
	
	

addLog("Process README.txt");
	
	/// Open the README.txt file and search for the release notes file
	$fp=fopen('README.txt','r')	;if (!$fp)											failProcess($JOB_ID."006",'Unable to open README.txt');
	$NEW_RELEASE='';
	while(!feof($fp))
	{
		$line=stream_get_line($fp,1000,"\n");
		/// There should be a release note file, from whihc we extract the year
		if (!preg_match('/Release ([0-9]{1,3})/',$line,$matches))continue;
	...
	fclose($fp);
	if (!unlink($W_DIR.'/README.txt'))												failProcess($JOB_ID."008",'Unable to remove README.txt');
...
addLog("Compare release");
	...
	if ($CURR_RELEASE!=-1 && $CURR_RELEASE!=getCurrentReleaseDate('ALFA',$JOB_ID))
	{
...
addLog("Update release tag");
	updateReleaseDate($JOB_ID,'NEW-ALFA',$NEW_RELEASE);
