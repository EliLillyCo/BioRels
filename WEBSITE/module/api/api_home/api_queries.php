...
function getTranscriptSequenceRange($TRANSCRIPT_NAME, $START_POS=-1, $END_POS=-1, $TYPE='RNA')
{
    ...
    foreach ($TMP as $K => $POS_INFO) {
        ...
        $DATA_TRANSCRIPT[$POS_INFO['transcript_id']]['SEQUENCE'][$POS_INFO['seq_pos']] = $POS_INFO;
    }
...
# // Description: Each parameter can contain one to multiple values, separated by a comma
# // Description: Example: 'GENE_SYMBOL=TP53,CDK2;DISEASE_TAG=MONDO_0005087' will search for all sentences mentioning MONDO_0005087 and either TP53 or CDK2 genes.
# // Description: Example: 'GENE_SYMBOL=TP53;GENE_SYMBOL=CDK2' will search for all sentences mentioning TP53 and CDK2 genes.
# // Parameter: PARAMS | Parameters | multi_array | multi_option | required
# // Multi-array: PARAMS | GENE_SYMBOL | Gene symbol | string | TP53 | optional
# // Multi-array: PARAMS | GENE_ID | Gene ID | string | 1017 | optional
# // Multi-array: PARAMS | DISEASE_TAG | Disease tag | string | MONDO_0005087 | optional
# // Multi-array: PARAMS | DISEASE_NAME | Disease name | string | Breast cancer | optional
# // Multi-array: PARAMS | CLINICAL_TRIAL | Clinical trial ID | string | NCT00000000 | optional
# // Multi-array: PARAMS | CELL_LINE | Cell line | string | MCF7 | optional
# // Multi-array: PARAMS | ANATOMY_TAG | Anatomy tag | string | UBERON_0002107 | optional
# // Multi-array: PARAMS | ANATOMY_NAME | Anatomy name | string | Breast | optional
# // Multi-array: PARAMS | DRUG | Drug name | string | Aspirin | optional
# // Return: Full text record
# // Ecosystem: Scientific_community:publication
...
# // $[API]
# // Title: Find news articles mentioning a set of parameters
# // Function: search_news
# // Description: Given a set of parameters, find news articles mentioning them
# // Description: Each parameter can contain one to multiple values, separated by a comma
# // Description: Example: 'GENE_SYMBOL=TP53,CDK2;DISEASE_TAG=MONDO_0005087' will search for all sentences mentioning MONDO_0005087 and either TP53 or CDK2 genes.
# // Description: Example: 'GENE_SYMBOL=TP53;GENE_SYMBOL=CDK2' will search for all sentences mentioning TP53 and CDK2 genes.
# // Parameter: PARAMS | Parameters | multi_array | multi_option | required
# // Multi-array: PARAMS | AUTHOR | Last name | string | Desaphy | optional
# // Multi-array: PARAMS | YEAR | Year | string | 2024 | optional
# // Multi-array: PARAMS | SOURCE | Source name | string | ClinicalTrials | optional
# // Multi-array: PARAMS | IS_QUATERLY_REPORT | Focus on quaterly report only | boolean | false | optional
# // Multi-array: PARAMS | COMPANY | Company name | boolean | false | optional
# // Multi-array: PARAMS | GENE_SYMBOL | Gene symbol | string | TP53 | optional
# // Multi-array: PARAMS | GENE_ID | Gene ID | string | 1017 | optional
# // Multi-array: PARAMS | DISEASE_TAG | Disease tag | string | Breast cancer | optional
# // Multi-array: PARAMS | DISEASE_NAME | Disease name | string | Breast cancer | optional
# // Multi-array: PARAMS | CLINICAL_TRIAL | Clinical trial ID | string | NCT00000000 | optional
# // Multi-array: PARAMS | DRUG | Drug name | string | Aspirin | optional
# // Ecosystem: Scientific_community:news
# // Example: php prep_queries.php search_news -PARAMS 'YEAR=2024;COMPANY=Pfizer'
# // $[/API]
function search_news($PARAMS)
{
	$COLS=array('news_title','news_content','news_release_date','source_name','wu.last_name','wu.first_name');
	$TABLES=array('news ne LEFT JOIN web_user wu ON wu.web_user_Id = ne.user_id
	LEFT JOIN source s ON s.source_id = ne.source_id');
	$JOINS=array();
	$data=array('PARAMS'=>array());
	$IDS=array();
	$N_T=0;
	...
	foreach ($PARAMS as $KEY=>&$T_LIST)
	{
		...
		switch ($KEY)
		{
			case 'AUTHOR':
				++$N_T;
				$IDS['rule_'.$N_T]['NAME']='Author_rule_'.$N_T;
				$TABLES[]='web_user wu'.$N_T;
				$COLS[]='wu'.$N_T.'.last_name as rule_'.$N_T;
				$COLS[]='wu'.$N_T.'.first_name as rule_'.$N_T;
				$JOINS[]='wu'.$N_T.'.web_user_Id = ne.user_id';
				$JOINS[]='LOWER(wu'.$N_T.'.last_name) IN (\''.strtolower(implode("','",$LIST)).'\')';
				$data['PARAMS'][$N_T]=array_unique($LIST);
				break;
			case 'YEAR':
				foreach ($LIST as $L)
				{
					if (!is_numeric($L)) throw new Exception('Year must be numeric');
					if ($L<1980 && $L>2050) throw new Exception('Year must be between 1980 and 2050');

				}
				$JOINS[]='EXTRACT(YEAR from news_release_date)  IN ('.implode(",",$LIST).')';
				break;
			case 'IS_QUATERLY_REPORT':
				
				$JOINS[]=' (LOWER(news_title) LIKE \'%quaterly%\' OR LOWER(news_title) LIKE \'%earnings%\')';
				break;
			case 'SOURCE':
				$res=runQuery("SELECT source_name FROM source WHERE lower(source_name) LIKE ('%".implode("%','%",array_map('strtolower',$LIST))."%')");
				if ($res==array()) return array();
				$JOINS[]=' source_name  IN (\''.implode("','",array_column($res,'source_name')).'\')';
				break;
			case 'GENE_SYMBOL':
				$LG=array();$SG=array();
				++$N_T;
				...
				if ($LG==array())return array();
					$TABLES[]='news_gn_map pgm'.$N_T;
					
					$COLS[]='pgm'.$N_T.'.gn_entry_id as rule_'.$N_T;
					$JOINS[]='pgm'.$N_T.'.news_id = ne.news_id';
					$JOINS[]='pgm'.$N_T.'.gn_entry_id IN ('.implode(',',$LG).')';
					$data['PARAMS'][$N_T]=array_unique($SG);
			...
			case 'DISEASE_TAG':
				...
				foreach ($LIST as $L)
				{
					...
					$DS=array_unique(array_column($res,'disease_entry_id'));
					$TABLES[]='news_disease_map pgm'.$N_T;
			...
			case 'COMPANY':
				++$N_T;
				$IDS['rule_'.$N_T]['NAME']='Company_rule_'.$N_T;
				foreach ($LIST as $L)
				{
					$res=search_company_by_name($L);
					if ($res==array()) return array();
					
					foreach ($res as $l)
					{
						$IDS['rule_'.$N_T][$l['company_entry_id']]=$l['company_name'];
					}
					$DS=array_unique(array_column($res,'company_entry_id'));
					$TABLES[]='news_company_map pgm'.$N_T;
					$COLS[]='pgm'.$N_T.'.company_entry_id as rule_'.$N_T;
					$JOINS[]='pgm'.$N_T.'.news_id = ne.news_id';
					$JOINS[]='pgm'.$N_T.'.company_entry_id IN ('.implode(',',$DS).')';
					$data['PARAMS'][$N_T]=array_unique(array_column($res,'company_name'));	
				}
				break;
			case 'DISEASE_NAME':
				++$N_T;
			...
			case 'CLINICAL_TRIAL':
				...
				{
					$INFO=get_clinical_trial_information($L);
					$NAMES=array();$DS=array();
					//echo '<pre>';print_R($INFO);
					foreach ($INFO as $l)
					{
						
							$IDS['rule_'.$N_T][$l['clinical_trial_id']]=$l['trial_id'];
							$DS[]=$l['clinical_trial_id'];$NAMES[]=$l['trial_id'];
						
					}
					
					$TABLES[]='news_clinical_trial_map pgm'.$N_T;
					

					$COLS[]='pgm'.$N_T.'.clinical_trial_id as rule_'.$N_T;
					$JOINS[]='pgm'.$N_T.'.news_id=ne.news_id';
					$JOINS[]='pgm'.$N_T.'.clinical_trial_id IN ('.implode(',',$DS).')';
					$data['PARAMS'][$N_T]=$NAMES;
				}
				break;
			...
			case 'DRUG':
				...
				//	echo '<pre>';print_R($LT);
					if ($LT==array()) return array();
					$DR_NAMES=array();$DS_ID=array();
					foreach ($LT as $LTL)
					{
						foreach ($LTL as $l)
						{
							$IDS['rule_'.$N_T][$l['drug_entry_id']]=$l['drug_primary_name'];
							$DR_NAMES[]=$l['drug_primary_name'];
							$DS_ID[]=$l['drug_entry_id'];
						}
					}
					
					$DS=array_unique($DS_ID);
					$TABLES[]='news_drug_map pgm'.$N_T;
					
					$COLS[]='pgm'.$N_T.'.drug_entry_id as rule_'.$N_T;
					$JOINS[]='pgm'.$N_T.'.news_id=ne.news_id';
					$JOINS[]='pgm'.$N_T.'.drug_entry_id IN ('.implode(',',$DS).')';
					$data['PARAMS'][$N_T]=array_unique($DR_NAMES);
	...
	foreach ($tmp as $DT)
	{
		$rec=array('news_title'=>$DT['news_title'],'news_content'=>$DT['news_content'],'news_release_date'=>$DT['news_release_date'],'source_name'=>$DT['source_name'],'last_name'=>$DT['last_name'],'first_name'=>$DT['first_name']);
...
{
	$query="SELECT company_entry_id, company_name FROM company_entry WHERE lower(company_name) LIKE '%".strtolower($COMPANY_NAME)."%'";
	return runQuery($query);
}
