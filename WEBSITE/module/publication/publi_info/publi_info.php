...
switch ($MODULE_DATA['TYPE'])
    {
        case 'DRUG':
			...
			$STR_I='';
			 ...
			 $STR.='</tbody></table>';
			 changeValue("publi_info","LIST_IMGS",$STR_I);
            break;
        ...
        case 'GENE':
			...
			<thead><tr><th>Symbol</th><th>Gene ID</th><th>Name</th></tr></thead><tbody>';
             foreach ($MODULE_DATA['RESULT'] as $G=>$R)
			 {
				...
				switch ($R['CONFIDENCE'])
				{
					case 5:$STR.='⋆⋆⋆</span>';break;
					case 4:$STR.='⋆⋆</span>⋆';break;
					default:$STR.='⋆</span>⋆⋆';break;
				}
				
				$STR.='</div></td><td style="vertical-align:middle;text-align:center"><a href="/GENEID/'.$G.'" target="_blank">'.$G.'</a></td><td style="vertical-align:middle;text-align:center">'.$R['NAME'].'</td></tr>';
				if ($R['DESCRIPTION'][0]!='')
				{
				$STR.='<tr style="border-top: hidden;
				border-bottom: 3px solid grey;"><td colspan="3"><span style="font-weight:bold; margin-left:20px;margin-right:10px">Description:</span><span style="text-align:justify; line-height:1.5em">';
        ...
        case 'DISEASE':
			...
			<tr><th>Disease Tag</th><th>Name</th><th style="max-width:60%;width:60%">Description</th></tr></thead><tbody>';
             foreach ($MODULE_DATA['RESULT'] as $G=>$R)
			 {
				$STR.='<tr><td style="vertical-align:middle;text-align:center"><a href="/DISEASE/'.$G.'" target="_blank">'.$R['DISEASE_TAG'].'</a><div style="    position: relative;
				cursor: pointer;
				top: -10px;height:10px"><span style="color:gold">';
				switch ($R['CONFIDENCE'])
				{
        ...
        case 'TISSUE':
			...
			<tbody>';
             foreach ($MODULE_DATA['RESULT'] as $G=>$R)
			 {
				$STR.='<tr><td style="vertical-align:middle;text-align:center"><a href="/TISSUE/'.$G.'" target="_blank">'.$G.'</a>
				<div style="    position: relative;
				cursor: pointer;
				top: -10px;height:10px"><span style="color:gold">';
				switch ($R['CONFIDENCE'])
	...
	changeValue("publi_info","INFO",$STR);
