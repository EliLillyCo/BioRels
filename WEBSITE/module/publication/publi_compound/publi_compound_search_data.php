...
if (isset($USER_INPUT['PORTAL']['DATA']['SM_ENTRY_ID'])) {
	...
	$MODULE_DATA['STAT'] = getCountPubliCompound($CP_ENTRY_ID, $FILTERS);
	...
	if ($MODULE_DATA['STAT']['CO'] != 0) {
		...
		$time = microtime_float();
		$MODULE_DATA['RESULTS'] = getPubliFromCompound($CP_ENTRY_ID, array('MIN' => ($PAGE - 1) * $PER_PAGE, 'MAX' => ($PAGE) * $PER_PAGE), $FILTERS);
	}
