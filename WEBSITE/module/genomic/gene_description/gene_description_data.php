...
$MODULE_DATA['UNIPROT']=geneToUniprot($USER_INPUT['PORTAL']['DATA']['GN_ENTRY_ID']);
if ($MODULE_DATA['UNIPROT']!=array()){
$PRIM_UN=$MODULE_DATA['UNIPROT'][0]['PROT_IDENTIFIER'];
$MODULE_DATA['UNIPROT_DESC']=getUniprotDescription($PRIM_UN,false);
}
$MODULE_DATA['INFO']=getGeneDescription($USER_INPUT['PORTAL']['DATA']['GN_ENTRY_ID']);
...
if (isset($MODULE_DATA['INFO']['DOCS']))
{
   ...
   foreach ($MODULE_DATA['INFO']['DOCS'] as &$DC_LIST)
   {
	foreach ($DC_LIST as $FORMAT=>$DC)
	{

	   if ($FORMAT!='text/xml')continue;

	   echo '<pre>';

	   $str=getNewsfile($DC['DOCUMENT_HASH'])['DOCUMENT_CONTENT'];
