...
changeValue("tissue_disease_view","TISSUE",$USER_INPUT['PORTAL']['VALUE']);


$co=$MODULE_DATA['STAT'];
//foreach ($MODULE_DATA['STAT'] as $IN)$co+=$IN['CO'];
changeValue("tissue_disease_view","COUNT",$MODULE_DATA['STAT']);
changeValue("tissue_disease_view","NPAGE",(ceil($co/10)));
...
foreach ($MODULE_DATA['RESULTS'] as $TYPE=>&$LIST)
{
	$STR='';
...
changeValue("tissue_disease_view",$TYPE,$STR);
}
