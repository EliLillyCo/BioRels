...
// 	}
	...
	changeValue("sequence_alignment","SEQUENCE",implode("\n",$STR));
	$NBLOCK=1;
	$STRHEAD='';
	$STRBL='';
	...
	{
		++$NBLOCK;
		$STRHEAD.='<li  id="seq_al_${MD5}_tab_'.$NBLOCK.'" class="nav-item nav-link" onclick="showMenu(\'seq_al_${MD5}\','.$NBLOCK.',${N_TOT})">'.ucfirst(strtolower($K)).'</li>';
		
		$STRBL.='<div id="seq_al_${MD5}_view_'.$NBLOCK.'" class="container-grey sequence w3-container w3-col s12 pre" style="display:none" >';
		$STRBL.=implode("\n",$V);
		$STRBL.='</div>';
	}
	
	changeValue("sequence_alignment","NBLOCK",count($STR_INT)+1);
	changeValue("sequence_alignment","LI_BLOCKS",$STRHEAD);
	changeValue("sequence_alignment","LIST_BLOCKS",$STRBL);
	changeValue("sequence_alignment","MD5",$ALL_NAME);
	changeValue("sequence_alignment","N_TOT",$NBLOCK);
	changeValue("sequence_alignment","LIST",json_encode ($TTL_LIST));
	/*
	...
	$STR_INT[$LINE_ID]=$NAME;
		...
		for ($T=$START;$T<$END;++$T)
		{
			...
			if ($TYPE=='REF')
			{
				...
				{
				$STR_INT[$LINE_ID].= '<span style="font-weight:bold;color:#'.$COLORS[$MODULE_DATA['REF_SEQ']['SEQ'][$V]].'"';
				...
				$STR_INT[$LINE_ID].= '>'.$MODULE_DATA['REF_SEQ']['SEQ'][$V].'</span>';
				}
				else $STR_INT[$LINE_ID].= '-';
				$STR[$LINE_ID].= '<span style="font-weight:bold;color:#'.$COLORS[$MODULE_DATA['REF_SEQ']['SEQ'][$V]].'">'.$MODULE_DATA['REF_SEQ']['SEQ'][$V].'</span>';
			}
			else 
			{
				if (isset($INTER_POS[$T]))
