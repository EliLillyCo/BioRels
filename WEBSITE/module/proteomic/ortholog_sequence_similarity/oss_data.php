...
$TMP=getOrthologAlignment($MODULE_DATA['REF_SEQ']['PROT_SEQ_ID']);
if (isset($TMP[0]))
{
$MODULE_DATA['REF_SEQ']['SEQ']=$TMP[0];
$MODULE_DATA['REF_SEQ']['INTER']=$TMP[2];
foreach ($TMP[1] as $SEQ_ID=>$INFO)
{
	if ($SEQ_ID=='SEQ')continue;
	$MODULE_DATA['ALT'][$SEQ_ID]['ALIGNMENT']=$INFO;
	$MODULE_DATA['ALT'][$SEQ_ID]['INFO']=$TMP[1]['SEQ'][$SEQ_ID];
}
