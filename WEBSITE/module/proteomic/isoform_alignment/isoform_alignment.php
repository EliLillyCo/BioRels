...
{
	changeValue("isoform_alignment","SEQUENCE",$USER_INPUT['PAGE']['VALUE']);
	removeBlock("isoform_alignment","VALID");
	return;
}
removeBlock("isoform_alignment","INVALID");
...
foreach ($MODULE_DATA['ALIGNMENT'] as $TYPE=>&$LIST)
{
	...
	else
	{
		...
		$T=$D['INFO']['ISO_ID'].' '.$D['INFO']['PROT_IDENTIFIER'];
		$NAME=$T;
		$ALL_NAME.=$NAME;
		foreach ($LIST as $AL_POS=>$DO_POS)
		{
...
$fs = 16;					/// fs and fc set for Courier New
	...
	$N_PER_LINE = floor(floor(($fc*$WIDTH / $fs) -$MAXHEAD-7-15)/10)*10;
...
$ALL_NAME=substr(md5('isoform_'.$ALL_NAME),10);
$LINES=ceil($LEN_AL/$N_PER_LINE);$POS=1;



changeValue("isoform_alignment","MD5",$ALL_NAME);

$TTL_LIST=array();$N_TTL=0;

#EBEBEB
for ($IL=0;$IL<$LINES*(2+count($MODULE_DATA['ALIGNMENT']));++$IL)
{
	$STR[$IL]="";
...
// 	}
	...
	changeValue("isoform_alignment","SEQUENCE",implode("\n",$STR));
	$NBLOCK=1;
	$STRHEAD='';
	$STRBL='';
	foreach ($STR_INT as $K=>$V)
	{
		...
		$STRHEAD.='<li id="iso_al_'.$ALL_NAME.'_tab_'.$NBLOCK.'" class="nav-item nav-link" onclick="showMenu(\'iso_al_'.$ALL_NAME.'\','.$NBLOCK.','.(count($STR_INT)+1).')">'.ucfirst(strtolower($K)).'</li>'."\n";
		
		$STRBL.='<div id="iso_al_'.$ALL_NAME.'_view_'.$NBLOCK.'" class="sequence w3-col s12 w3-container pre" style="display:none" >';
		
		$STRBL.=implode("\n",$V);
		$STRBL.='</div>';
	}
	
	changeValue("isoform_alignment","NBLOCK",count($STR_INT)+1);
	changeValue("isoform_alignment","LI_BLOCKS",$STRHEAD);
	changeValue("isoform_alignment","LIST_BLOCKS",$STRBL);
	changeValue("isoform_alignment","LIST",json_encode ($TTL_LIST));
